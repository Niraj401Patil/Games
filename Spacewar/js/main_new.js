...
//********************************instantiate the game objects******************************************************/
   ...
   let rocket1 = new rocket(0,context1.height/2,compSize*3,compSize*1.5,context1,imageRocket);
   ...
   let addObstacles=setInterval(function(){stages.addObstacles(level*10*2)},speed*2);
   let shipUp;
   let shipDown;        
   let flag=true;
          ...
          // alert("no");
           shipDown= setInterval(function(){
               for(let i in stages.ships){
                ...
                stages.ships[i].addBomb(stages.ships[i].x-stages.ships[i].width,stages.ships[i].y+stages.ships[i].height/2);
               }
           },1000);
...
//***********************************add buttons on canvas for game control***************************************************/
   let btnMiddle=new button(compSize*2,canv.height-compSize*4,compSize*2,compSize*2,"","blue","");
   let btnLeft=new button(0,canv.height-compSize*4,compSize*2,compSize*2,"","gray","");
   let btnRight=new button(compSize*4,canv.height-compSize*4,compSize*2,compSize*2,"","gray","");
   let btnUp=new button(compSize*2,canv.height-compSize*6,compSize*2,compSize*2,"","gray","");
   let btnDown=new button(compSize*2,canv.height-compSize*2,compSize*2,compSize*2,"","gray","");
   let btnFire=new button(canv.width/2,canv.height-compSize*3,compSize*3,compSize*3,"f","gray","yellow");
...
//********************************check  bullet shut to obstacle******************************************************/
    ...
    function isBullet_Clash_Ship(){
        for(let j in stages.ships){      
            for(let i in rocket1.bullet){
              if((rocket1.bullet[i].y>=stages.ships[j].y&&rocket1.bullet[i].y<=stages.ships[j].y+stages.ships[j].height)&&rocket1.bullet[i].x+40>=stages.ships[j].x && rocket1.bullet[i].x<=stages.ships[j].x){
           
                    rocket1.bullet[i].x=context1.width+10;
                    score+=10;
                    if(shipLive<=0){
                        soundDestroy();
                        drawBlast(imageBlast, stages.ships[j].x+10,stages.ships[j].y,stages.ships[j].width+20,stages.ships[j].height+20); 
                        stages.ships.pop();
                        score+=100;
                    }
                    shipLive-=1;
...
//***************************************check  rocket clash with obstacle***********************************************/
    function isRocket_Clash_obstacle(){
        for(let j in stages.obstacles){      
            if((stages.obstacles[j].x-rocket1.width/1.2<=rocket1.x&&stages.obstacles[j].x>=rocket1.x)&&(stages.obstacles[j].y>=rocket1.y-rocket1.height&&stages.obstacles[j].y<=rocket1.y+rocket1.height)){
    ...
    function isRocket_Clash_Bomb(){
        for(let i in stages.bomb){      
           if((stages.bomb[i].y>=rocket1.y&&stages.bomb[i].y<=rocket1.y+rocket1.height)&&stages.bomb[i].x-rocket1.width<=rocket1.x && stages.bomb[i].x>=rocket1.x){
                stages.bomb[i].x=-200;
                soundDestroy();
                drawBlast(imageBlast, rocket1.x,rocket1.y-10,rocket1.width+10,rocket1.height+10);
                rocket1.x=0;
